...
namespace MyLibrary
{
    public class Gene1s//基因一：決策樹編碼，進出場策略基因 (固定天期)
    {
        ...
        int[] RncValue = new int[4] { 5, 10, 15, 20 };//固定天期：5、10、15、20
        ...
        public int[] RncInitial()//RNC陣列初始化
        {
            for (int i = 0; i < rnc.Length; i++)
                rnc[i] = RncValue[rnd.Next(0, 4)];//固定天期
            return rnc;
        }
        ...
        public int[] RNCMutation(int[] rnc)//RNC陣列1~3點突變
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                rnc[position[i]] = RncValue[rnd.Next(0, 4)];
            }
            return rnc;
        }
